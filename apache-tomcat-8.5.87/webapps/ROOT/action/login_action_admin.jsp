...
try {
    ...
    if (rs1.next()) {
        if(rs1.getString("password").equals(password)){
            ...
            out.println("<script language=javascript>self.window.alert('패스워드가 일치하지 않습니다.'); location.href='../login_admin.jsp';</script>");
        };

        
    } else {
         out.println("<script language=javascript>self.window.alert('존재하지 않는 정보입니다.'); location.href='../login_admin.jsp';</script>");
         
    }
    rs1.close();
} catch (SQLException e) {
    out.println("<script language=javascript>self.window.alert('로그인 실패'); location.href='../login_admin.jsp';</script>");
    e.printStackTrace();
}
